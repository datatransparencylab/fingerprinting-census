...
d3.json("https://privacymeter-dtl.firebaseio.com/insights.json", function (json) {
    ...
    var totalAnalyzed = insightData.total_sites;
    totalAnalyzed = d3.format(",")(totalAnalyzed);
    totalFingerprintedTraffic = d3.format(",")(totalFingerprintedTraffic);

    var lastUpdated = insightData.last_update;
    ...
    visualizeDataT();
        ...
        drawDoughnuts("categories", "Business and Industry", "#category1");
        drawDoughnuts("categories", "News and Media", "#category2");
        drawDoughnuts("categories", "Health", "#category3");
        drawDoughnuts("categories", "Career and Education", "#category4");
        drawDoughnuts("categories", "Computer and Electronics", "#category5");
...
function visualizeDataT() {
    ...
    total_sites = insightData.total_sites;
    fingerprint_sites = insightData.tracking_sites;
...
function drawBargraph() {
    ...
    d3.json("https://privacymeter-dtl.firebaseio.com/insights/top_ranks/top_categories_traffic.json", function (error, data1) {
        //d3.json("https://privacymeter-dtl.firebaseio.com/data/global/top_ranks/top_categories_traffic.json", function (error, data1) {
        ...
        gr.selectAll(".bar")
            ...
            .on("mousemove", function (d) {
                ...
                tooltip
                    ...
                    .html((d.category) + "<br><span>" + (d3.format(",")(d.traffic)) + "   " + "tracked visits" + "</span>");
            })
...
function drawDoughnuts(query, target, catDiv) {
    ...
    var total = insightData[query][target].websites;
    // 2- get # of fingerprint news sites
    var tracking = insightData[query][target].websites_tracking;

    var traffic = insightData[query][target].traffic_tracking;
    ...
    var colorMap = {
        "Arts and Entertainment": d3.rgb(204, 0, 190),
        "Health": d3.rgb(0, 190, 204),
        "Business and Industry": d3.rgb(0, 204, 14),
        "News and Media": d3.rgb(116, 0, 204),
        "Computer and Electronics": d3.rgb(18, 152, 241),
        "Shopping": d3.rgb(0, 204, 116),
        "Home and Garden": d3.rgb(116, 255, 0),
        "Career and Education": d3.rgb(204, 0, 88)
    };
    ...
    var iconMap = {
        "Arts and Entertainment": "images/icons/film-strip-with-two-photograms.svg",
        "Health": "images/icons/medical-kit.svg",
        "Business and Industry": "images/icons/briefcase.svg",
        "News and Media": "images/icons/newspaper.svg",
        "Computer and Electronics": "images/icons/open-laptop-computer.svg",
        "Shopping": "images/icons/shopping-cart-black-shape.svg",
        "Home and Garden": "images/icons/home.svg",
        "Career and Education": "images/icons/smile.svg"
    }
    ...
    d3.select(catDiv)
        ...
        .html("<span>" + perc + "%" + "</span>" + "  of " + target + "  sites are fingerprinting." + "<br><span>" + parseInt(traffic) + "</span>" + "  " + " visits to " + target + " sites were fingerprinted in a month.");
    // " amounting to " +
...
function drawTrendsGraph(dataset, divId) {
    var margin = {
            top: (parseInt(d3.select(divId).style('width'), 10) / 10),
            right: (parseInt(d3.select(divId).style('width'), 10) / 20),
            bottom: (parseInt(d3.select(divId).style('width'), 10) / 5),
            left: (parseInt(d3.select(divId).style('width'), 10) / 20)
        },
